...
class MainGridFragment : Fragment() {
    ...
    private lateinit var listPair : Pair<MutableList<CommonItems>, List<CommonItems>>
    private lateinit var displayList: List<CommonItems>
    private lateinit var sortedList: List<CommonItems>
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val layoutManager = GridLayoutManager(requireContext(), 3) // 최대 3개의 항목이 한 줄에 표시
        layoutManager.spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
            override fun getSpanSize(position: Int): Int {
                return when (recyclerview.adapter?.getItemViewType(position)) {
                    BFMainListViewType.Category -> 3  // 한 줄에 한 개만 표시
                    else -> 1  // 기본값
                }
            }
        }
        ...
        //스와이프 이벤트 부착
        itemTouchHelper = ItemTouchHelper(MainGridListItemHelper(requireActivity()))
        itemTouchHelper.attachToRecyclerView(binding.mainRecyclerview)

        displayList = displayData()
        listAdapter.addItems(displayList)

        listAdapter.itemClick = object : MainGridListAdapter.ItemClick {
        ...
        override fun bindHeaderData(header: View?, headerPosition: Int) {
            if(headerPosition != RecyclerView.NO_POSITION && isHeader(headerPosition)){
                header?.findViewById<TextView>(R.id.tvName)
                    ?.text = ((binding.mainRecyclerview.adapter as MainGridListAdapter).getItem()[headerPosition] as MainCategory).str
            }
        }
        override fun isHeader(itemPosition: Int): Boolean {
            return if(itemPosition != RecyclerView.NO_POSITION){
                (binding.mainRecyclerview.adapter as MainGridListAdapter).getItemViewType(itemPosition) == BFMainListViewType.Category
            } else {
                false
            }
        }
